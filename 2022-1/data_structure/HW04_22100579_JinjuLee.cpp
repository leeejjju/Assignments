...
class mystack{
    ...
    int pop(); //하나 빼내기
    int show(); //가장 최근에 들어간 놈을 (제거하지 않고)반환. 
    bool stack_empty(); //비어있는지 체크
    bool stack_full(); //꽉차있는지 체크 

};

int calculatePriority(char x); //오퍼레이터나 하여튼 이것저것 받아서 우선순위 반환 
...
int main(){
    ...
    for (i = 0; i < sizeof(myFormula); i++ ){ //스택에 수식 한자한자씩... 걍 넣으면 안되는구나 
        ...
        if(myFormula[i] == '('){
            ...
            while(1){ //최상위원소 우선순위가 본인보다 높은 한 
                if(calculatePriority(s1.show()) > calculatePriority(myFormula[i])){
                    cout << s1.pop(); //꺼내서 출력 
                }else{ //낮아지면?
    ...
    while ( ! s1.stack_empty()){ //스택에 쌓인놈들을 하나씩 빼냄(후입선출)
