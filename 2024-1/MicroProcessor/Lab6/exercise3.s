...
.equ GPIOTE_CONFIG2_OFFSET, 0x518 // GPIOTE channel 2
.equ GPIOTE_EVENT_IN0, 0x40006100
.equ GPIOTE_EVENT_IN1, 0x40006104
.equ GPIOTE_EVENT_IN2, 0x40006108

.equ AIRCR_BASE, 0xE000ED0C

/*
change to support the Soft Reset by Button3 push event
in order to Write the register VECTKEY shold be set to 0x05FA 
*/
...
GPIO_SETUP:
  ...
  // Button123 setting
  LDR R1, =0x0003000c
  STR R1, [R0, #GPIO_PIN_CNF_24_OFFSET]
...
GPIOTE_SETUP:
  ...
  MOV R1, #(0x01+0x02+0x4) // channel = 0
  ...
  // btn3 -> PIN_CNF[24] configurations
  // for mode setting and pin number setting
  MOV R1, #0x01 // mode = event
  MOV R2, #24   // pin number = 12
  LSL R2, R2, #8 // shift bits to PSEL place (BBBBB)
  ORR R1, R1, R2
  // for port number setting
  MOV R2, #0x00 // port number = 0
  ...
  LSL R2, R2, #16 // shift bits to PORT place (DD)
  ORR R1, R1, R2
  // setting GPIOTE_PIN_CONFIG[24]
  STR R1, [R0, #GPIOTE_CONFIG2_OFFSET]
...
GPIOTE_Handler:
  ...
  //******************btn3*******************
  LDR R0, =GPIOTE_EVENT_IN2
  LDR R3, [R0] // Read GPIOTE Event In1
  //*****************************************
  ...
  BEQ btn3 // If No Button2 Event, Skip this area 
...
reset:
  ...
  ldr r1, [r0]
  orr r1, r1, #(1 << 2) //set SYSRESETREQ bit to 1
  ldr r2, =0x0000FFFF
  and r1, r1, r2
  ldr r2, =0x05FA0000 //set VECTKEY register
  orr r1, r1, r2
  str r1, [r0]
  b .
