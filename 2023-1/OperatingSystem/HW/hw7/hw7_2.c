...
// TO DO: complete this function following instructions
{
	...
	if((pt->frame = (int*)malloc(sizeof(int)*(pt->no_page))) == NULL){
		printf("error: cannot allocate memory for frames.\n");
		return;
	}

	// TO DO: for safety, initialize the elements of pt->frame by filling it with zeroes
	memset(pt->frame, 0, sizeof(pt->frame));

}

void PageTable_Destroy(PageTable *pt)
{
	// TODO: deallocate pt->frame and set all fields with zeros
	free(pt->frame);
...
// load page table from a file
{
	...
	int pagenum_bits, offset_bits, process_size;
...
// Translate a logical address into physical address
{
	...
	int page_no, offset, frame_no;

	page_no = logical >> pt->offset_bits; 

	offset = logical & pt->offset_mask;

	frame_no = pt->frame[page_no];
	...
	#ifdef _DEBUG
	printf("[DEBUG] logical = %x, page_no = %x, offset = %x, frame_no = %x, physical = %x\n", logical, page_no, offset, frame_no, physical);
	#endif
